            ...
            @forelse($products as $product)
                @foreach($product->variants as $variant)
                    <div wire:key="variant-card-{{ $variant->id }}">
                        <!-- Product Card -->
                        <button wire:click="selectProduct({{ $variant->id }})"
                            wire:loading.class="opacity-50 cursor-wait"
                            wire:loading.attr="disabled"
                            wire:target="selectProduct({{ $variant->id }})"
                            ...
                            <div class="aspect-square bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 flex items-center justify-center p-6 relative overflow-hidden">
                                ...
                                <div class="absolute top-3 right-3">
                                    ...
                                    @endphp
                                    <span class="px-3 py-1.5 rounded-full text-sm font-bold shadow-lg backdrop-blur-sm {{ $storeStock > 10 ? 'bg-green-500/90 text-white' : ($storeStock > 0 ? 'bg-orange-500/90 text-white' : 'bg-red-500/90 text-white') }}">
                                        {{ $storeStock }}
                                    </span>
                                </div>
                            </div>
                            <div class="p-5">
                                <h3 class="font-bold text-base text-gray-900 truncate mb-2 group-hover:text-indigo-600 transition-colors">{{ $product->name }}</h3>
