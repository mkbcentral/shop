    ...
    <form wire:submit.prevent="nextStep" class="space-y-6">
        ...
        <div>
            <label class="block text-sm font-medium text-slate-300 mb-4">
                Type d'activité <span class="text-red-400">*</span>
            </label>
            <p class="text-xs text-slate-400 mb-4">
                Sélectionnez le type d'activité de votre organisation. Cela déterminera les types de produits et catégories disponibles.
            </p>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                @foreach($businessActivities as $activity)
                    <label class="relative cursor-pointer group" wire:key="activity-{{ $activity['value'] }}">
                        <input
                            type="radio"
                            wire:model.live="business_activity"
                            value="{{ $activity['value'] }}"
                            name="business_activity"
                            class="peer sr-only"
                            @if($business_activity === $activity['value']) checked @endif
                        >

                        {{-- Card with selection state --}}
                        <div class="relative p-4 rounded-xl border-2 transition-all duration-300
                            @if($business_activity === $activity['value'])
                                border-indigo-500 bg-gradient-to-br from-indigo-500/20 to-purple-500/10 shadow-lg shadow-indigo-500/20
                            @else
                                border-slate-700 bg-slate-800/50 hover:border-indigo-400/50 hover:bg-slate-800/80 group-hover:scale-[1.01]
                            @endif">

                            {{-- Selection indicator --}}
                            <div class="absolute top-2 right-2 transition-opacity duration-300 @if($business_activity === $activity['value']) opacity-100 @else opacity-0 @endif">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="text-sm font-semibold text-white truncate group-hover:text-indigo-300 transition-colors">
                                        {{ $activity['label'] }}
                                    </h4>
        ...
        <div class="flex gap-4 pt-2">
            ...
            <button
                ...
                class="flex-1 inline-flex justify-center items-center px-6 py-3.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600
            ...
            >
                <svg wire:loading wire:target="nextStep" class="animate-spin h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24">
