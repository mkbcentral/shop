<div x-data="{ showModal: false, showDeleteModal: false, clientToDelete: null, clientName: '', isEditing: false }"
        ...
        <x-card>
            <x-slot:header>
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-semibold text-gray-900">Liste des Clients ({{ $clients->total() }})</h2>
                    <div class="w-72">
                        <x-form.search-input
                                    ...
                                    @if ($sortField === 'name')
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M{{ $sortDirection === 'asc' ? '5 15l7-7 7 7' : '19 9l-7 7-7-7' }}" />
                                        </svg>
                                    @endif
                                </div>
                            </th>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <button @click="$wire.openEditModal({{ $client->id }})"
                                        ...
                                        <button @click="showDeleteModal = true; clientToDelete = {{ $client->id }}; clientName = '{{ $client->name }}'"
                    ...
                    <div class="flex-1 overflow-y-auto p-6 min-h-0">
                        <div class="space-y-5">
                            ...
                            <x-form.form-group label="Nom du client" for="name" required>
                                <x-form.input wire:model="name" id="name" type="text" placeholder="Ex: Jean Dupont, Société ABC..." icon="user" />
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <!-- Phone Field -->
                                <x-form.form-group label="Téléphone" for="phone">
                                    <x-form.input wire:model="phone" id="phone" type="text" placeholder="Numéro de téléphone" icon="phone" />
                                    <x-form.input-error for="phone" />
                                </x-form.form-group>

                                <!-- Email Field -->
                                <x-form.form-group label="Email" for="email">
                                    <x-form.input wire:model="email" id="email" type="email" placeholder="Adresse email" icon="mail" />
                                    <x-form.input-error for="email" />
                                </x-form.form-group>
                            </div>

                            <!-- Address Field -->
                            <x-form.form-group label="Adresse" for="address">
    ...
    <x-delete-confirmation-modal show="showDeleteModal" itemName="clientName" itemType="le client"
        onConfirm="$wire.delete(clientToDelete); showDeleteModal = false; clientToDelete = null; clientName = ''"
        onCancel="showDeleteModal = false; clientToDelete = null; clientName = ''" />
</div>
