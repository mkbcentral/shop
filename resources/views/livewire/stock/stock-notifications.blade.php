    ...
    >
        ...
        @if($this->totalAlertsCount > 0)
            <span class="absolute -top-1 -right-1 flex items-center justify-center min-w-[20px] h-5 px-1 text-xs font-bold text-white bg-red-500 rounded-full animate-pulse">
                {{ $this->totalAlertsCount > 99 ? '99+' : $this->totalAlertsCount }}
            </span>
        @endif
    </button>
    ...
    >
        ...
        <div class="max-h-[400px] overflow-y-auto">
            ...
            @else
                ...
                @if($this->storeSummary->isNotEmpty())
                    <div class="px-4 py-3 bg-gray-50 border-b">
                        <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">
                            Résumé par magasin
                        </h4>
                        <div class="space-y-2">
                            @foreach($this->storeSummary as $summary)
                ...
                <div class="divide-y divide-gray-100">
                    ...
                    @foreach($this->stockAlerts->take(10) as $alert)
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between">
                                        <p class="text-sm font-medium text-gray-900 truncate">
                                            {{ $alert->product?->name ?? 'Produit inconnu' }}
                                        </p>
                                        <span class="ml-2 text-xs font-semibold {{ $alert->stock_quantity <= 0 ? 'text-red-600' : 'text-amber-600' }}">
                                            {{ $alert->stock_quantity }} unité{{ $alert->stock_quantity != 1 ? 's' : '' }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="mt-1">
                                        ...
                                        @else
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-800">
                                                Stock bas (seuil: {{ $alert->low_stock_threshold }})
                                            </span>
                                        @endif
                                    </div>
                                </div>
                ...
                @if($this->totalAlertsCount > 10)
                    <div class="px-4 py-2 bg-gray-50 text-center">
                        <span class="text-xs text-gray-500">
                            Et {{ $this->totalAlertsCount - 10 }} autre(s) alerte(s)...
                        </span>
                    </div>
                @endif
            @endif
